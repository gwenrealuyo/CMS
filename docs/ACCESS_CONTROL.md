...
| **Evangelism Coordinator** (non-senior) | **Search/list:** same-branch people for member picking. **Profile:** self + family (plus other module scopes) | No expanded Families list | N/A | Create extra groups (pending until Senior / Pastor / Admin approve); add/remove members on create, edit, and group view for managed groups (pending or approved); **visitors** and **conversions** only on managed approved groups; **reports** and **tally** only for groups they can file for; **Each 1 Reach 1** create/edit for those groups’ clusters; **Bible Sharers** tab only if on the HQ roster |
...
**Evangelism Coordinator** (non-senior): **read** all approved groups in their branch; **create** extra groups (pending until Evangelism Senior / Pastor / Admin approve); **edit** only managed groups and their own pending drafts; **add/remove members** on create, edit, and group view for those groups (pending or approved) without a prior Cluster or evangelism-group resource; **add visitors** and **record conversions** only on managed approved groups; **reports** and **tally** only for groups they can file for; **Each 1 Reach 1** create/edit for clusters of those groups; **Bible Sharers** tab only if on the HQ roster.
...
6. **Vital dates**: `date_first_invited`, `date_first_attended`, `first_activity_attended`, baptism dates, and lessons-started / lessons-finished fields are editable only by **cluster coordinator+** (ADMIN, PASTOR, CLUSTER Senior Coordinator, or CLUSTER Coordinator for a cluster containing that person). Others see locked fields with a “contact your cluster coordinator” hover hint. **First Activity Attended** lists ministry event types only (`counts_as_activity`); Meeting room holds are excluded.
