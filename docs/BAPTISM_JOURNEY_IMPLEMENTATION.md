...
- **Description**: Optional descriptive text (e.g., "First attendance")
- **User**: The Person instance
- **Verified by**: None (system-generated)
...
1. Use `pre_save` signal to store original values in instance (e.g., `_original_water_baptism_date`, `_original_spirit_baptism_date`, `_original_date_first_attended`)
2. In `post_save`, compare original vs current values
3. For each date field:
   - If date was added/changed: Create or update journey
   - If date was cleared: Delete journey if exists
4. Use the date directly for the Journey's `date` field (no formatting needed in title)

## Testing Considerations

- Test creating Person with date fields already set
