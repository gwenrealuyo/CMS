...
- `apps.attendance.models.AttendanceRecord` links an `Event` to a `Person` for a specific occurrence (`occurrence_date`) and stores the attendance `status` (`PRESENT`, `ABSENT`, `EXCUSED`). Each record automatically synchronises with a `Milestone` of type `EVENT_ATTENDANCE`, ensuring the person’s timeline reflects their event participation.
...
- `POST /api/events/{id}/attendance/` accepts `person_id`, `occurrence_date`, optional `status`, and upserts an attendance record. Each successful write syncs the matching `EVENT_ATTENDANCE` milestone.
- `DELETE /api/events/{id}/attendance/{attendance_id}/` removes the attendance record and its milestone.
...
- On the frontend, `EventView` includes an Attendance panel that:
  ...
  - lets coordinators add/remove attendees; status defaults to “Present” and edits sync milestones automatically;
  - surfaces the total recorded attendees and highlights whether milestones are logged.
...
Attendance and milestone flows are exercised by API tests in `apps/attendance/tests/test_attendance_api.py`.
