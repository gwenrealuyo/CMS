...
## Progress & Milestone Synchronisation

- `apps.lessons.services.mark_progress_completed` generates the `LESSON` milestone when a progress record transitions to `COMPLETED`. Rolling a lesson back triggers `revert_progress_completion`.
- Commitment form signatures call `mark_commitment_signed`, which writes a `NOTE`-type milestone titled “Commitment Form Signed” and timestamps the signer. Reverting the checkbox removes the milestone through `revert_commitment_signed`.
...
- Participants’ progress rows expose a “Signed” toggle that opens a confirmation modal; confirming patches the `commitment_signed` flag and triggers milestone bookkeeping server-side.
...
- `/api/lessons/lessons/` – CRUD for lesson definitions (includes nested milestone config).
...
- `LessonDetailPanel` defaults the “Milestone Settings” and “Metadata” cards to collapsed, with toggles to reveal details, keeping the UI focused on actionable info.
...
- There are currently no automated tests in `apps.lessons.tests`. When adding coverage, focus first on:
  - Progress completion + milestone creation/reversion.
  - Commitment signature flow (including milestone toggling).
